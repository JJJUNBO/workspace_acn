				...
				if(prev_len[x]!=len){
					...
					while(lanfile.getFilePointer()!=len){
						...
						if(line.contains("DV")){
							...
							System.out.println("data message "+receiver[1]);
							...
							for(int n=0;n<=9;n++){
								if(receiver[n]==1&&lan_id!=x){
									...
									RandomAccessFile lanfile2 = new RandomAccessFile("lan"+nextlan[n],"rw");
									
									lanfile2.seek(lanfile2.length());
									
									lanfile2.writeBytes("data "+nextlan[n]+" "+host_id+"\n");
